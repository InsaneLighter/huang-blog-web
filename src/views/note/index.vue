    ...
    <div class="note_container">
      <div class="noteList">
        <div class="skeleton" v-if="isLoading">
          <a-skeleton v-for="index of 5" :key="index" active/>
        </div>
        <a-list item-layout="vertical" size="large" :pagination="pagination" :data-source="notes" v-else>
...
export default {
  ...
  methods: {
    ...
    handleLike (item) {
      item.likes += 1
      let likesArr = JSON.parse(window.localStorage.getItem('huang_blog_note_like')) || []
      likesArr.push(item.id)
      window.localStorage.setItem('huang_blog_note_like', JSON.stringify(likesArr))
      try {
        journalApi.like({
          id: item.id,
          like: true
